...
型参照サイトのtext差分では、同じ宣言が複数の型を参照する場合、その宣言を前後それぞれ一度だけ表示します。削除・追加の下の役割と型表記は同じ宣言に属します。完全な参照サイトはJSONで確認できます。

観測できる変更・できない変更の境界は、`python3 Scripts/review_probes.py`で合成15ケースを再実行できます。結果は`.build/review-probes.json`へ保存します。Actionsでも実行し、成果物の`probes.json`へ保存します。成功は期待終了コードの確認であり、意味的な網羅性の合格ではありません。

PRの解析が完了すると、Sekkaのbotコメントを1件作成・更新します。変更ファイルと構造案内、比較SHA、解析の限界、成果物リンクを確認できます。コメント更新は別の権限を限定したworkflowで実行し、PRのコードへ書込tokenを渡しません。チェック失敗時は結果更新失敗の案内になります。

PRコメントとActions要約の構造案内は、`├─`・`└─`・`│`の罫線で型→観測→詳細を表します。記号は階層を示し、重要度や設計の良否を表しません。CLIの生出力はartifactにそのまま保存します。
