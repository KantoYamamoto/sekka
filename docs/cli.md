...
行指定には入力IDが必須です。コメント追加や作業ツリー変更も含め、前回の解析入力と異なる場合は終了コード2で停止するので、案内を取り直してください。同一実行内では取得済みソースからdiffを作ります。一意なpropertyのbinding範囲や、追加・削除メンバーの片側範囲からも選べます。対応が曖昧・同じ行に複数bindingがある・トップレベル関数等の未対応構文なら、ファイル全体へ戻ったことを明示します。

選択数/全hunk数を表示します。hunkには隣接する変更や文脈も含まれ、宣言境界で切り取りません。新規ファイルの全追加行が一つのhunkなら、宣言選択が成功しても全行を表示し、その理由を明示します。GitHub/JSON形式での`--show-diff`は受け付けません。
...
- 型の識別はファイルパス＋種別＋入れ子の名前＋同名宣言の出現順です。移動・改名は削除と追加です。extensionは独立の記録とし、元の型には統合しません。出現順はsnapshot内の識別用であり、同名宣言が複数あって両側に存在するときは構造・本体の前後比較を省略し、変更ファイルと理由を案内します。宣言群全体が片側だけなら追加・削除を表示します。
