...
worktreeのuntrackedはindexでの不存在にすぎないため、baseにもあるパスはobject hashとmodeを照合し、同一なら省略・違えばmodifiedとする（レビューで追加）。Gitのindex hintや正規化でGit一覧に出なくても、解析済みSwiftのraw source差分がある場合は一覧へ補い、scopeに明記する。coverageの変更ファイルがinventoryから消えないことを保証する。非Swiftの内容比較はGitの規則に従う。
