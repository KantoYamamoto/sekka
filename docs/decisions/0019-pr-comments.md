...
PRコメントとジョブ要約を共通のレンダラーで生成する。変更・構造観測・構造観測なし・比較省略を区別し、ファイルリンクを先に、全文案内を折りたたみに置く。人間が開く場所を選べることが目的。ユーザーの補足に従い絵文字ではなく、`├─`・`└─`・`│`と字下げで型→観測→詳細を表す。ラベルと事実は変更しない。

## Why not

pushごとの新コメントは議論を埋めるため採らない。罫線を重要度や設計の合否として使わない。PRビルドへ投稿用tokenを渡さず、成果物中のMarkdownやコードをそのまま実行・投稿しない。ローカル用CLIのrunner絶対パスはコピーしても使えないため、画面の案内から除き、完全artifactは保持する。
