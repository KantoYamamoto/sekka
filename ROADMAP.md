...
**現在は#52の対照検証を完了。次は同じ呼び出し表記の配置が広がる事実と既存位置を、実験として機械抽出できるか検証する。人間の判断待ちは解除。** [0031](docs/decisions/0031-structural-success.md)に従い、「読む場所が分かる」だけでは成功としない。役割設定を先に要求する0030は撤回した。
...
| 最小の機械抽出 | #52で論点の根拠になった事実に限り、前後・曖昧さ・対照の試験と自己利用を実施 | #52の結果後にIssueと範囲を決定。カードをそのまま製品化しない |
| 未読実変更の検証 | 自動抽出した材料で、変更一覧から構造の問いへ進めたかを独立比較 | 抽出精度を確認した後。既知例の再読を代用しない |
